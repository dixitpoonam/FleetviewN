...
//	con1 = DriverManager.getConnection(MM_dbConn_STRING,MM_dbConn_USERNAME,MM_dbConn_PASSWORD);
    ...
    String sql=""; 
    String oldusername = request.getParameter("oldmanagername");
    String usertypevalue=session.getAttribute("usertypevalue").toString();
	String newuser ="";
	System.out.println("olduser   "+oldusername);
	String buffer="";
	int i=1;
	String FirstName="",LastName="",Name="",Manghodid="",MangerName="-",Rsmhodid="",RsmName="-";
 ...
 //sql="select UserName,Email from db_gps.t_userdetails where Transporter like '%"+usertypevalue+"%' and UserName like '%"+oldusername+"%' limit 20";
	...
	sql="select hodid from db_gps.t_userdetails where UserName='"+oldusername+"' ";
	System.out.println("sql:---"+sql);
	rs2=stmt2.executeQuery(sql);
	if(rs2.next()){
		Manghodid=rs2.getString("hodid");
	}
	
	String sql2="select UserName,hodid from db_gps.t_userdetails where SrNo='"+Manghodid+"' ";
	rs3=stmt3.executeQuery(sql2);
	if(rs3.next())
	{
		MangerName=rs3.getString("UserName");
    ...
    String sql3="select UserName from db_gps.t_userdetails where SrNo='"+Rsmhodid+"' ";
