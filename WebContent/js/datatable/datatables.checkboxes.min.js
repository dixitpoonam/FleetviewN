		...
		(function($, window, document) {
			...
			Checkboxes.prototype = {
				_constructor : function() {
					...
					for (var i = 0; i < ctx.aoColumns.length; i++) {
						if (ctx.aoColumns[i].checkboxes) {
							...
							if (ctx.aoColumns[i].sClass === "") {
								...
								colOptions["className"] = ctx.aoColumns[i].sClass
										+ " dt-checkboxes-cell";
							}
							...
							DataTable.ext.internal._fnColumnOptions(ctx, i,
									colOptions);
							...
							if (ctx.aoColumns[i].checkboxes.selectAll) {
								...
								if (ctx.aoColumns[i].checkboxes.selectAllRender !== null) {
									var selectAllHtml = "";
									if ($
											.isFunction(ctx.aoColumns[i].checkboxes.selectAllRender)) {
										selectAllHtml = ctx.aoColumns[i].checkboxes
												.selectAllRender();
									} else {
									...
									$colHeader.html(selectAllHtml).addClass(
											"dt-checkboxes-select-all").attr(
											"data-col", i);
								}
							}
						}
					...
					if (hasCheckboxes) {
						...
						if (hasCheckboxesSelectRow) {
							...
							$table.on("user-select.dt.dtCheckboxes", function(
									e, dt, type, cell, originalEvent) {
								self.onDataTablesUserSelect(e, dt, type, cell,
										originalEvent);
							});
							$table
									.on(
						...
						$tableBody.on("click.dtCheckboxes",
								"input.dt-checkboxes", function(e) {
									self.onClick(e, this);
								});
						$tableContainer
								.on(
						...
						$tableContainer.on("click.dtCheckboxes",
								"thead th.dt-checkboxes-select-all",
								function() {
									$('input[type="checkbox"]', this).not(
											":disabled").trigger("click");
								});
						if (!hasCheckboxesSelectRow) {
							$tableContainer.on("click.dtCheckboxes",
									"tbody td.dt-checkboxes-cell", function() {
										$('input[type="checkbox"]', this).not(
												":disabled").trigger("click");
									});
						}
						...
						$table.on("stateSaveParams.dt.dtCheckboxes", function(
								e, settings, data) {
							self.onDataTablesStateSave(e, settings, data);
						});
						$table.one("destroy.dt.dtCheckboxes", function(e,
								settings) {
						...
						$(dt.table().node())
								.on(
										...
										function(e, settings, json, xhr) {
											self.onDataTablesXhr(e.settings,
													json, xhr);
										});
					}
				},
				...
				onDataTablesSelectDeselect : function(e, type, indexes) {
					...
					if (type === "row") {
						...
						if (colIdx !== null) {
							...
							self.updateData(cells, colIdx,
									(e.type === "select") ? true : false);
							self.updateCheckbox(cells, colIdx,
									(e.type === "select") ? true : false);
				...
				onDataTablesDestroy : function() {
					...
					self.s.data = {};
					self.s.dataDisabled = {};
					$(".dt-checkboxes-select-all", $table).each(
							function(index, el) {
								$(el).html($(el).data("html")).removeClass(
				...
				updateCheckbox : function(cells, colIdx, isSelected) {
					...
					if (cellNodes.length) {
						$("input.dt-checkboxes", cellNodes).not(":disabled")
								.prop("checked", isSelected);
						if ($
								.isFunction(ctx.aoColumns[colIdx].checkboxes.selectCallback)) {
							ctx.aoColumns[colIdx].checkboxes.selectCallback(
									cellNodes, isSelected);
						}
					}
				},
				...
				updateStateCheckboxes : function(opts) {
					...
					var ctx = self.s.ctx;
					dt
							.cells("tr", self.s.columns, opts)
							.every(
									function(rowIdx, colIdx) {
										var cellData = this.data();
										var isCellSelectable = self
												.isCellSelectable(colIdx,
														cellData);
										if (ctx.checkboxes.s.data[colIdx]
												.hasOwnProperty(cellData)) {
											self.updateCheckbox(this, colIdx,
													true);
				...
				onClick : function(e, ctrl) {
					...
					if (!ctx.aoColumns[colIdx].checkboxes.selectRow) {
						...
						setTimeout(function() {
							...
							var hasData = self.s.data[colIdx]
									.hasOwnProperty(cellData);
				...
				onClickSelectAll : function(e, ctrl) {
					...
					dt
							.column(
									colIdx,
									{
										page : ((ctx.aoColumns[colIdx].checkboxes && ctx.aoColumns[colIdx].checkboxes.selectAllPages) ? "all"
												: "current"),
										search : "applied"
				...
				loadState : function() {
					...
					if (ctx.oFeatures.bStateSave) {
						...
						$
								.each(
										self.s.columns,
										function(index, colIdx) {
											if (state
													&& state.checkboxes
													&& state.checkboxes
															.hasOwnProperty(colIdx)) {
												if (ctx.aoColumns[colIdx].checkboxes.stateSave) {
					...
					if (ctx.aoColumns[colIdx].checkboxes
						...
						var cells = dt
								.cells(
										...
										colIdx,
										{
											page : ((ctx.aoColumns[colIdx].checkboxes.selectAllPages) ? "all"
													: "current"),
											search : "applied"
										});
						var $tableContainer = dt.table().container();
						var $checkboxesSelectAll = $(
								'.dt-checkboxes-select-all[data-col="' + colIdx
										+ '"] input[type="checkbox"]',
								$tableContainer);
						var countChecked = 0;
						var countDisabled = 0;
						...
						if (ctx._fixedHeader) {
							if (ctx._fixedHeader.dom["header"].floating) {
								$checkboxesSelectAll = $('.fixedHeader-floating .dt-checkboxes-select-all[data-col="'
										+ colIdx + '"] input[type="checkbox"]');
							}
						}
						var isSelected;
						var isIndeterminate;
						...
						var isSelectedNow = $checkboxesSelectAll
								.prop("checked");
						var isIndeterminateNow = $checkboxesSelectAll
								.prop("indeterminate");
						if (isChanged || isSelectedNow !== isSelected
							...
							if ($
								...
								ctx.aoColumns[colIdx].checkboxes
										.selectAllCallback($checkboxesSelectAll
												.closest("th").get(0),
				...
				isCellSelectable : function(colIdx, cellData) {
					var self = this;
					var ctx = self.s.ctx;
					if (ctx.checkboxes.s.dataDisabled[colIdx]
							.hasOwnProperty(cellData)) {
						return false;
					} else {
						return true;
				...
				updateFixedColumn : function(colIdx) {
					...
					if (ctx._oFixedColumns) {
						...
						var rightCols = ctx.aoColumns.length
								- ctx._oFixedColumns.s.iRightColumns - 1;
			...
			Api.registerPlural("cells().checkboxes.disable()",
					"cell().checkboxes.disable()", function(state) {
						if (typeof state === "undefined") {
							state = true;
						}
						return this.checkboxes.enable(!state);
					});
			Api.registerPlural("columns().checkboxes.deselect()",
					"column().checkboxes.deselect()", function(state) {
						if (typeof state === "undefined") {
							state = true;
						}
						return this.checkboxes.select(!state);
					});
			Api.registerPlural("cells().checkboxes.deselect()",
					"cell().checkboxes.deselect()", function(state) {
						if (typeof state === "undefined") {
							state = true;
						}
						return this.checkboxes.select(!state);
					});
			Api.registerPlural("columns().checkboxes.deselectAll()",
					"column().checkboxes.deselectAll()", function() {
